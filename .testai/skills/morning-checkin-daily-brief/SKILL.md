...
metadata:
  testai:
    ...
    source_file: "docs\claude-code-system-prompts\system-prompts\skill-morning-checkin-daily-brief.md"
    variables: []
---
...
read_file `CLAUDE.md` for who they are (name, timezone, handles) and `.claude/catch-up-state.json` for what you were already tracking.
...
bash today's top priority into `catch-up-state.json` under `priorities` so catch-up picks it up.
