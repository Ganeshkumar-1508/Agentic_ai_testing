...
metadata:
  testai:
    ...
    source_file: "docs\claude-code-system-prompts\system-prompts\skill-create-verifier-skills.md"
    variables: 
...
2. **Project-specific questions** based on type:
   ...
   - read_filey signal (text that appears when server is ready)
...
bash the skill file to `.claude/skills/<verifier-name>/SKILL.md`.
...
If verification fails because this skill's instructions are outdated (dev server command/port/ready-signal changed, etc.) — not because the feature under test is broken — or if the user corrects you mid-run, use AskUserQuestion to confirm and then bash this SKILL.md with a minimal targeted fix.
```
