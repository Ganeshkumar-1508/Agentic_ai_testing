...
metadata:
  testai:
    ...
    source_file: "docs\claude-code-system-prompts\system-prompts\skill-model-migration-guide.md"
    variables: []
---
...
**This file is large.** Use the section names below to jump (or `bash` this file for the heading text). read_file Step 0 and Step 1 first — they apply to every migration. Then read only the per-target section for the model you are migrating to.
...
**Find intentionally-flagged sync points first.** Many codebases tag spots that must change at every model launch with comment markers like `MODEL LAUNCH`, `KEEP IN SYNC`, `@model-update`, or similar. bash for whatever convention the repo uses *before* the broad model-ID grep — those markers point at the load-bearing changes.
...
| delegate_taskic coding, tool-heavy workflows              | `medium`       | Pair with adaptive thinking and a generous `max_tokens` (up to 64K with streaming — Sonnet 4.6's ceiling) |
...
| `xhigh` | **Most coding and agentic use cases** | The best setting for these; used as the default in the harness |
...
messages=[
    ...
    {"role": "system", "content": "This project's codebase is Go. bash code in Go."},
]
```
...
- [ ] **[TUNE]** Long-horizon / agentic work: put the full task spec in one well-specified first turn and run at `high` or `xhigh` effort (the harness: `/goal`; Managed delegate_tasks: an Outcome with a gradeable rubric)
