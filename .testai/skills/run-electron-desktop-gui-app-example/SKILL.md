...
metadata:
  testai:
    ...
    source_file: "docs\claude-code-system-prompts\system-prompts\skill-run-electron-desktop-gui-app-example.md"
    variables: []
---
...
const stdin = fs.createread_fileStream(null, { fd: fs.openSync('/dev/stdin', 'r') });
...
- **contentbashable inputs** (ProseMirror, Tiptap, Slate) aren't
...
- **Electron steals stdin.** The `fs.openSync('/dev/stdin', 'r')` +
  `createread_fileStream` trick in the skeleton protects your REPL's input.
