"""OSV malware check for MCP extension packages (adapted from Hermes).
...
Adapted from: reference/hermes-agent/tools/osv_check.py (Apache 2.0)
"""
...
def check_package_for_vulnerabilities(
    package: str, ecosystem: str, version: Optional[str] = None
) -> list[dict]:
    """Check a package for known vulnerabilities (not just malware).

    Returns list of vulnerability dicts with id, summary, severity.
    """
    try:
        ...
        logger.debug("OSV vulnerability check failed for %s/%s: %s", ecosystem, package, exc)
        return []
...
def _parse_package_from_args(
    ...
    if ecosystem == "npm":
        ...
        return _parse_rubygems_package(package_token)
    elif ecosystem == "crates.io":
        return _parse_cargo_package(package_token)
...
def _parse_rubygems_package(token: str) -> Tuple[Optional[str], Optional[str]]:
    """Parse RubyGems package: name:version or name."""
    if ":" in token:
        parts = token.split(":", 1)
...
def _parse_cargo_package(token: str) -> Tuple[Optional[str], Optional[str]]:
    """Parse Cargo package: name:version or name."""
    if ":" in token:
        parts = token.split(":", 1)
        return parts[0], parts[1]
    return token, None
...
def _query_osv(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "testai-agent-osv-check/1.0",
        },
        method="POST",
    )
