...
as tool-output truncation thresholds. The same hardcoded constants
appeared in two places upstream:
...
def get_tool_output_limits() -> Dict[str, int]:
    ...
    Keys: ``max_bytes``, ``max_lines``, ``max_line_length``. Currently
    always returns the ``DEFAULT_*`` constants — TestAI's user
    configuration lives in PostgreSQL (:mod:`harness.memory.settings_store`),
    not in a ``config.yaml`` file. If a future TestAI ``~/.testai/config.yaml``
    is added, wire a ``tool_output`` block through the same
    :func:`get_tool_output_limits` shape. This function NEVER raises.
    """
    return {
