...
def _global_allow_private_urls() -> bool:
    ...
    try:
        ...
        if isinstance(sec, dict) and str(sec.get("allow_private_urls", "")).lower() in ("true", "1", "yes"):
            _cached_allow_private = True
            return _cached_allow_private
        # browser.allow_private_urls (legacy fallback)
        browser = cfg.get("browser", {})
        if isinstance(browser, dict) and str(browser.get("allow_private_urls", "")).lower() in ("true", "1", "yes"):
            _cached_allow_private = True
            return _cached_allow_private
    except Exception:
