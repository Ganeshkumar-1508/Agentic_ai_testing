...
Trust levels:
  - builtin:   Ships with TestAI. Never scanned, always trusted.
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/\.(hermes|testai)/\.env|\~/\.(hermes|testai)/\.env',
     "agent_secrets_access", "critical", "exfiltration",
     "directly references an agent's secrets file (Hermes .hermes/.env or TestAI .testai/.env)"),
    ...
    (r'\.(hermes|testai)/config\.yaml|\.(hermes|testai)/SOUL\.md',
     "agent_config_mod_home", "critical", "persistence",
     "references agent configuration files directly (Hermes ~/.hermes or TestAI ~/.testai)"),
