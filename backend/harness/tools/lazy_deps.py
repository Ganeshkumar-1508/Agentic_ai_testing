"""Lazy dependency installer for opt-in feature backends.

Many features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
Slack, Matrix, etc.) require Python packages that not every user needs.
The historical approach was to bundle them all under
``pyproject.toml`` extras and install them eagerly at setup time. That
has two problems:
...
remediation hint pointing at the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    Defaults to True. The opt-out is exposed via the ``TESTAI_DISABLE_LAZY_INSTALLS``
    env var (previously ``HERMES_DISABLE_LAZY_INSTALLS``) so the same
    fail-open posture is preserved when no user config is present.
    Refusing to install would lock people out of their own backends; the
    decision to block is an explicit user opt-in.
    """
    if os.environ.get("TESTAI_DISABLE_LAZY_INSTALLS") == "1":
        return False
    return True
...
def _is_satisfied(spec: str) -> bool:
    ...
    the update flow propagate pin bumps in :data:`LAZY_DEPS` to
    already-installed backends instead of silently leaving stale
    versions in place.
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Kept independent of any CLI module so callers don't drag a
    whole command parser in just to install a single extra.
    """
    if not specs:
...
def active_features() -> list[str]:
    ...
    Used by the update flow to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for the update flow. Never raises; lazy-install failures
