...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/testai-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/testai-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/testai-results"
HEREDOC_MARKER = "TESTAI_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/testai-results"
    return STORAGE_DIR
