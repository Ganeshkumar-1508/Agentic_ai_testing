"""Cross-agent file state coordination (adapted from Hermes).
...
check — this module catches the case where subagent B writes a file that
subagent A already read, so A's next write would overwrite B's changes.
...
wrap the whole read→modify→write block.

Adapted from: reference/hermes-agent/tools/file_state.py (Apache 2.0)
"""
...
# long sessions from accumulating unbounded state.
_MAX_PATHS_PER_AGENT = 4096

# Global last-writer map cap.
...
class FileStateRegistry:
    ...
    def check_stale(self, task_id: str, resolved: str) -> Optional[str]:
        ...
        # Case 3: never read AND no write record
